function denoise_BSPC_2011_main
...
% Load ground truth
img_truth = double(imread('MRI_truth.bmp'));
[nRow, nColumn] = size(img_truth);    

% Generate noisy image
noise_sig_truth = 0.2;
G=randn(nRow,nColumn)*noise_sig_truth + sqrt(-1)*randn(nRow,nColumn)*noise_sig_truth;
GL = nlfilter(G,[3 3],'mean(mean(x))');
GL=abs(GL);
m=mean(mean(GL));
GL=GL-m+1;
img_noise=GL.*img_truth;        
img_noise = round(img_noise);
img_noise(img_noise==0)=1;
img_noisy = log(img_noise);
...
img_denoised = exp(img_denoised);

% Calculate the PSNR performance
fprintf('PSNR=%.2fdB\n', func_psnr_gray(img_truth, img_denoised));

% Write the output image
imwrite(uint8(img_denoised), ['MRI_denoised.bmp'], 'bmp');
